Register-ArgumentCompleter -Native -CommandName @('code', 'code.cmd', 'code.exe') -ScriptBlock {
	...
	$LastCommand = $commandAst.CommandElements | Select-Object -Last 1
	if ($LastCommand -eq $wordToComplete) {
		$LastCommand = $commandAst.CommandElements | Select-Object -Last 2
	}
	# $completions += Get-Completions -WordToComplete $wordToComplete -LastCommand $LastCommand
